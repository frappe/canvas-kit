...
import NumberArrows from "./NumberArrows.vue";
...
const emitUpdate = useDebounceFn((value: string | boolean) => emit("update:modelValue", value), 100);

// read the value at event time; reading it when the debounce fires can pick up
// a re-rendered input showing another block's value
const triggerUpdate = ($event: Event) => {
	const target = $event.target as HTMLInputElement;
	emitUpdate(props.type === "checkbox" ? target.checked : target.value);
};
